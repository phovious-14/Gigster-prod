...
const Task = ({ task, toggleComplete, removeTask }: any) => {
    return (
        <div className="flex items-center mb-2 p-2 bg-[#ffffff5e] rounded w-full">
            <p className={`flex-grow ${task.completed ? 'line-through text-gray-500' : ''}`}>
                {task.text}
            </p>
            <button
                onClick={() => removeTask(task.id)}
                className="px-2 py-1 text-white rounded bg-[#00000080]"
            >
